...
type AirFlow
    ...
    | CubicMetersPerSecond Float


type Density
    = LbPerCubicFoot Float
...
type RelativeHumidityRate
    = HPercentPerHour Float


type SpecificGasConstant
    = JoulePerKgPerK Float
...
inCubicMetersPerSecond d =
    case d of
        CubicMetersPerSecond cms ->
            cms
...
inUnitsString sys temperature =
    case sys of
        Metric ->
            (inCelsius temperature |> roundn 1 |> toString) ++ "(°C)"

        Imperial ->
            (inFahrenheit temperature |> roundn 1 |> toString) ++ "(°F)"


inUnitsPower : SystemOfUnits -> Power -> Float
inUnitsPower sys power =
    case sys of
        Metric ->
            inKilowatts power
...
inUnitsPowerString sys power =
    case sys of
        Metric ->
            (inKilowatts power |> roundn 1 |> toString) ++ "(kW)"

        Imperial ->
            (inTons power |> roundn 1 |> toString) ++ " (tons)"


inUnitsAirflow : SystemOfUnits -> AirFlow -> Float
inUnitsAirflow sys airflow =
    case sys of
        Metric ->
            inCubicMetersPerSecond airflow

        Imperial ->
            inCubicFeetPerMinute airflow


inUnitsAirflowString : SystemOfUnits -> AirFlow -> String
inUnitsAirflowString sys airflow =
    case sys of
        Metric ->
            (inCubicMetersPerSecond airflow |> roundn 1 |> toString) ++ "(cubic m/s)"

        Imperial ->
            (inCubicFeetPerMinute airflow |> roundn 1 |> toString) ++ " (cfm)"
